...
namespace Core.Binder
{
    public class BindItem
    {
        ...
        public object GetInject(Type type)
        {
            object key = this;
            ...
            result = CreateInstance(key, type);
            _instanceDictionary.Add(key, result);
            return result;
        }

        private object CreateInstance(object key, Type type)
        {
            if (_isSingleton)
            {
                if (!_classTypeDictionary.ContainsKey(this))
                {
                    _classTypeDictionary.Add(this, type);
                }
            }
            ...
            if (!_classTypeDictionary.TryGetValue(key, out targetType))
            {
                Debug.LogError("BindManager : " + type.Name + " at Key: " + keyName + " inject not found!!!");
                return null;
            }
